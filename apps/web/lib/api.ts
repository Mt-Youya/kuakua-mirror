...
export const deviceApi = {
  ...
  // 获取设备状态
  getDeviceStatus: (deviceId: string) => request<any>(`/devices/${deviceId}/status`),
...
export const heartbeatApi = {
  ...
  // 获取心跳历史
  getHeartbeatHistory: (deviceId: string) => request<any[]>(`/heartbeat/${deviceId}/history`),
}
...
export const dataApi = {
  ...
  // 获取最新数据
  getLatestData: (deviceId: string) => request<any>(`/data/${deviceId}/latest`),

  // 获取历史数据
  getHistoricalData: (deviceId: string, params?: { limit?: number }) =>
    request<any[]>(`/data/${deviceId}/history${params?.limit ? `?limit=${params.limit}` : ""}`),
}
